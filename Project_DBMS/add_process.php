<?php
    ...
    function order($item,$con){
        ...
        $sql_insert = "INSERT INTO order_food(ID_market,ID_customer,status_ID,refer_code) 
                        VALUES(".$_SESSION['cart']['id_market'].",".$_SESSION['id_customer'].",0,'".$refer_code."')";
    ...
    $sql_status_market = "SELECT status_market FROM market WHERE ID_market = ".$_GET['id_market'];
    $result_status = mysqli_fetch_assoc(mysqli_query($connect,$sql_status_market));
    if($result_status['status_market'] == '1'){
        ...
        if(isset($_SESSION['id_customer'])){
            ...
            if($get_wallet['wallet_point'] < $total){
                ...
                $refer = order($_SESSION['cart']['item'],$connect);
        ...
        echo "<script>alert('Market is closed.');</script>";
        echo "<script>window.location.href = 'Homepage.php';</script>";
    }
