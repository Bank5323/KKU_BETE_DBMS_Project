...
<body>
    ...
    <div class="container">
        ...
        <div class="close">
            <?php
                $sql_status_market = "SELECT status_market FROM market WHERE ID_market = $id_market";
                $result_status = $connect->query($sql_status_market);
                $status = $result_status->fetch_assoc();
                if($status['status_market'] == '0'){
                    
                    echo "<p>ร้านค้าปิดอยู่</p>";
                    echo "<a href = 'Upstatus.php?id_market=$id_market&status=1'>เปิดร้าน</a>";
                }
                else{
                    echo "<p>ร้านค้าเปิดอยู่</p>";
                    echo "<a href = 'Upstatus.php?id_market=$id_market&status=0'>ปิดร้าน</a>";
                }
            ?>
        </div>

        <p class='title'>รายการอาหาร</p>
        <table class="tk">
            <thead>
                <tr>
    ...
    <!-- <?php
            ...
            if($status['status_market'] == '0'){
                ...
                echo "<p class='close_M'>ร้านค้าปิดอยู่</p>";
                echo "<a href = 'Upstatus.php?id_market=$id_market&status=1'>เปิดร้าน</a>";
            }
            else{
                echo "<p>ร้านค้าเปิดอยู่</p>";
